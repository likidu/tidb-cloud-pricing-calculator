  ...
  return (
    <div className="mx-auto max-w-6xl p-6 grid gap-6 md:grid-cols-2">
      <div className="space-y-4">
        <h1 className="text-2xl font-semibold">TiDB Cloud Pricing Calculator</h1>
        ...
        <div className="grid gap-4 bg-white rounded border p-4">
          ...
          <NumberInput
            ...
            step={0.01}
            onChange={(v) =>
              setInputs((s) => ({ ...s, baselineWorkloadPct: Math.max(0, Math.min(1, v || 0)) }))
            }
          />
        </div>
        ...
        <div className="grid gap-4 bg-white rounded border p-4">
          ...
          <NumberInput
            ...
            min={0}
            step={0.01}
            onChange={(v) => setInputs((s) => ({ ...s, storagePricePerGBMonth: v || 0 }))}
          />
          <div className="grid grid-cols-2 gap-4">
            <NumberInput
      ...
      <div className="space-y-4">
        <div className="grid gap-2 bg-white rounded border p-4">
          ...
          <Row label="Metering Storage Size (GB)" value={inputs.mysqlDataDirGB / 3 / inputs.compressionRatio} />
          <Row label="Peak RCU" value={inputs.vcpuPeak * inputs.rcuPerVcpu} />
          <Row label="Baseline RCU" value={inputs.vcpuBaseline * inputs.rcuPerVcpu} />
          <Row label="Amplitude" value={Math.max(0, (inputs.vcpuPeak - inputs.vcpuBaseline) * inputs.rcuPerVcpu)} />
          <Row label="Avg Consumed RCU" value={result.averaged.avgConsumedRcu} />
        </div>

        <div className="grid gap-2 bg-white rounded border p-4">
          <h2 className="font-medium">Starter (Serverless)</h2>
          <Row label="Storage Price" value={result.starter.storagePriceUsd} money />
          <Row label="Consumed RU (Million)" value={result.starter.consumedRuMillion} />
        ...
        <div className="grid gap-2 bg-white rounded border p-4">
          ...
          <Row label="Storage Price" value={result.essential.storagePriceUsd} money />
          <Row label="Provisioned RCU" value={result.essential.provisionedRcu} />
          <Row label="Provisioned RCU Price" value={result.essential.provisionedRcuPriceUsd} money />
          <Row label="Total" value={result.essential.totalUsd} money highlight />
        </div>
      </div>
    </div>
  )
...
function Row({ label, value, money, highlight }: { label: string; value: number; money?: boolean; highlight?: boolean }) {
  return (
    <div className={`flex items-center justify-between ${highlight ? 'font-semibold' : ''}`}>
      <span className="text-gray-700">{label}</span>
      <span>{money ? formatCurrency(value) : Number.isFinite(value) ? value.toLocaleString() : '-'}</span>
    </div>
  )
}
